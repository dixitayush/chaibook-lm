...
| `APP_URL` | for Google / VPS | App origin, e.g. `http://localhost:3000` or `https://chaibook.ayushdixit.work` |
...
The public edge is **Traefik** (`../gateway`). It is the only process that binds host ports 80/443. Chaibook’s production stack is **Postgres + Redis + the Next.js app + Caddy**, all unpublished. Traefik routes `CHAIBOOK_HOST` to Caddy on the private `proxy` network; Caddy proxies to the app (including SSE chat). Redis stores short-term memory with `allkeys-lru` and also backs rate limits (chat 25/min, studio 12/min, login 10/min IP, register 8/min IP; the API fails open if Redis is down).
